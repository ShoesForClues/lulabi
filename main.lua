...
fusion.scheduler:create_thread(function(thread)
	...
	elseif thread.libraries["stdlib"].root_functions.string.lower(arg[1])=="help" then
		...
		thread.platform:print("-c <compiler>                Select compiler (default g++ | configured from lulabi_make)")
		thread.platform:print("-std <c_standard>            Select C/C++ standard (default c++0x | configured from lulabi_make)")
